...
export default function LeaveRequestsPage() {
  ...
  const fetchLeaveRequests = useCallback(async () => {
    try {
      ...
      setError("Error fetching leave requests",err);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleStatusChange = useCallback(async (leaveRequestId, status, employeeId) => {
    try {
      ...
      setError("Error processing the leave request",err);
    }
  }, []);
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-4">Todays Leave Requests</h1>
      <div>
