...
//                // Update user details2 in context
//     updateUserDetails2({
//         userName: response.data.user.username,
//         employeeId: response.data.user.employeeId,
//         userEmail: response.data.user.email,
//         userRole: decodedToken.userrole,
//         managerEmail: response.data.user.managerEmail,
...
export default function LoginPage() {
  ...
  const onLogin = async () => {
    ...
    try {
      ...
      // Decode the token to extract user role
      const decodedToken = jwtDecode(token);
      const userRole = decodedToken.userrole; // Assuming the role is stored as 'userrole' in the token
