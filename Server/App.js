...
app.patch('/api/login', checkUserExists, async (req, res) => {
  ...
  try {
    ...
    // Ensure the user is signing in according to their shift, allowing a 15-minute early sign-in grace period
    if (action === 'Sign In') {
      // Check if the timestamp is before the shift start time by more than 15 minutes
      const now = moment(timestamp).tz('Asia/Kolkata'); // Current time in IST
      const shiftStart = moment(shift.startTime).tz('Asia/Kolkata');

      // Allow sign-in if it's within 15 minutes of the shift start time
      const earlySignInLimit = shiftStart.clone().subtract(15, 'minutes');

      if (now.isBefore(earlySignInLimit)) {
        return res.status(400).json({
          message: `You cannot sign in earlier than 15 minutes before your shift start time at ${startTime}`
        });
      }
    }
  ...
  try {
    ...
    switch (shiftType) {
      ...
      case 'nightshift':
        ...
        endTime = new Date(startTime);
        endTime.setHours(3, 0, 0, 0);   // 3:00 AM (next day)
        break;
      default:
        return res.status(400).json({
          success: false,
...
// Route for the manager to approve or decline an expense
app.put('/update-expense/:id', async (req, res) => {
  const { expenseId, managerId, managerEmail, status, reason } = req.body;
  const { id } = req.params;  // Extract the expense ID from URL parameter

  // Validate required fields
  if (!expenseId || !managerId || !managerEmail || !status) {
    return res.status(400).json({
      success: false,
      message: 'Please provide all required fields: expenseId, managerId, managerEmail, and status.',
    });
  }
  ...
  try {
    ...
    // If the status is 'Declined', ensure the reason is provided
    ...
    // If the status is 'Approved', set the manager's reason to 'Approved'
    if (status === 'Approved') {
      expense.managerReason = 'Approved';
...
// Route to fetch all expenses by employeeId
app.get('/getAll/expenses/employeeId', async (req, res) => { 
  try {
    const { employeeId } = req.query;

    // Filter expenses by employeeId if provided
    let filter = {};
    if (employeeId) {
    ...
    const allExpenses = await Expense.find(filter);
    if (allExpenses.length === 0) {
      return res.json({ success: false, message: 'No expenses found for the provided employee.' });
    }
...
// Get all uploaded expenses filtered by managerId
app.get("/get-files", async (req, res) => {
  try {
    const { managerId } = req.query; // Get managerId from query parameters

    // If managerId is provided, filter expenses by managerId
    const filter = managerId ? { managerId } : {}; // If managerId exists, filter by managerId, otherwise no filter

    const files = await Expense.find(filter); // Fetch expenses using the filter
...
app.get('/files/:filename', (req, res) => {
  ...
  const filePath = path.join(__dirname, 'files', filename);  // Ensure path points to the correct folder 'files'

  // Check if the file exists before attempting to send it
  res.download(filePath, (err) => {
    if (err) {
      console.error("Error downloading file:", err);
      res.status(500).json({ success: false, message: "Error downloading file", error: err });
    }
  });
});
